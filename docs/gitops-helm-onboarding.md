...
### 4. Crossplane Compositions

The Universal Helm Chart generates an `AINativeSaaS` XR, which Crossplane reconciles using one of two Compositions:

- **Composition A (Starter/Growth):** Shared Spoke Pool with dual-namespace isolation
- **Composition B (Premium/Enterprise):** Dedicated Spoke with dedicated CNPG cluster

See [Crossplane Compositions](./crossplane-compositions.md) for detailed XRD schemas, Composition implementations, and dual-plane network isolation patterns.
...
### 1. Helm vs. Crossplane Boundary
Helm generates ONLY the `AINativeSaaS` XR. Crossplane handles ALL infrastructure reconciliation (namespaces, NetworkPolicies, ResourceQuotas, databases). See [Crossplane Compositions](./crossplane-compositions.md).

### 2. Bootstrap Handoff Pattern
ClusterResourceSet injects ONLY the ArgoCD Agent (Secret Zero). ArgoCD ApplicationSets deploy the edge-catalog (CNPG, NATS, SPIRE, Alloy). See [Hub-Spoke Architecture](./hub-spoke-architecture.md).

### 3. Dual-Plane Network Isolation
Each tenant gets two namespaces with NetworkPolicy-enforced isolation. See [Crossplane Compositions](./crossplane-compositions.md) for detailed NetworkPolicy specifications.

### 4. Cell-Based Scaling
SpokePool XRs enable horizontal scaling from 100 to 10,000 tenants. See [Crossplane Compositions](./crossplane-compositions.md) for SpokePool XRD and Fleet Registry patterns.
